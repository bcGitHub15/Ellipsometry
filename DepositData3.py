...
NOTE that the x and y arrays start at the to right and scan left and
down! Actually it is worse than that, the x values zig-zag back and
forth across the space.
...
class DepositData3:
    def __init__(self, x, y, t1, t2=None, 
                 grading=None, roughness=None, voidness=None, name=None):
        self.name = name
...
#        print(f'ymax = {ymax}')
        ...
        self.xdata, self.ydata = DepositData3.new_grid(self.radius, self.dx)
...
#        print(f'Copied {mask_count} data points')
        #
        # Array placeholders
        #
        self.fit_z = None
    ...
    def new_with_mask(self, mask):
        ...
        result = DepositData3(newx, newy, newz, newn)
...
#            ax.scatter(self.xdata, self.ydata, c='k', marker='.', s=10)
    ...
    def from_file(filename, data_name = None):
        ...
        dd = DepositData3(arr[:, 0], arr[:, 1], arr[:, 2], arr[:, 5], arr[:, 3], arr[:, 4], arr[:, 6]) 
...
if __name__ == '__main__':
    s2 = DepositData3.from_file('Run6_26_Scan2.txt', 'Scan2')
    s2.Ron_plot()
